      ...
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <div className="flex items-center cursor-pointer group" onClick={() => onNavigate('home')}>
          ...
          <div className="flex items-center gap-3">
             ...
             <div className="flex flex-col justify-center">
                <span className="text-xl md:text-2xl font-black text-slate-900 leading-none tracking-tight group-hover:text-[#2563EB] transition-colors">
                   모두의 문제연구소
                </span>
             </div>
          </div>
          ...
          {/* Secondary: University Affiliation */}
        ...
        <div className="hidden md:flex items-center space-x-8 text-sm font-semibold">
           <div className="flex space-x-6 text-gray-600">
            <button onClick={() => onNavigate('activity')} className="hover:text-[#2563EB] transition-colors">리서치</button>
          ...
          <button 
            ...
            className="bg-[#2563EB] text-white px-6 py-2.5 rounded-full hover:bg-[#1d4ed8] transition-colors font-bold shadow-md shadow-blue-200 transform hover:-translate-y-0.5"
          >
            함께하기
          </button>
        </div>
      ...
      {isOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white border-b border-gray-100 p-6 flex flex-col space-y-4 shadow-xl font-semibold animate-fade-in-down z-50">
          <div className="pb-4 border-b border-gray-100">
             <span className="text-xs font-bold text-slate-400 uppercase tracking-widest block mb-2">Affiliation</span>
             <span className="text-sm font-bold text-slate-700">한양대 ERICA 수리데이터사이언스학과</span>
          </div>
          <button onClick={() => { onNavigate('activity'); setIsOpen(false); }} className="text-lg text-gray-600 text-left hover:text-[#2563EB]">리서치</button>
          ...
          <div className="pt-2">
            <button 
              onClick={() => { onNavigate('apply'); setIsOpen(false); }} 
              className="w-full bg-[#2563EB] text-white px-6 py-4 rounded-xl font-bold text-center shadow-lg shadow-blue-100"
            >
              함께하기
            </button>
          </div>
